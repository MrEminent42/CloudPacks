...
/*	public static CloudPack getPack(UUID id) {
		...
		return new CloudPack(UUID.fromString(file.getName()), cfg.getConfig().getString("name"), 
				UUID.fromString(file.getName()), items, cfg.getConfig().getInt("rows"), false, false);
	}*/
	...
	public static CloudPack getUnloadedPack(UUID id) {
		File file = new File("plugins/CloudPacks/storage" + id + ".yml");
		ConfigWrapper cfg = new ConfigWrapper(plugin, "plugins/CloudPack/storage/", file.getName());
		List<ItemStack> items = new ArrayList<ItemStack>();
		for (int i = 0; i < cfg.getConfig().getInt("rows"); i++) items.add(cfg.getConfig().getItemStack("contents." + i));
